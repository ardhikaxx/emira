...
@section('title', 'Edit Rekam Medis - EMIRA')
            ...
            <div class="card-header-custom">
                <div class="header-title">
                    ...
                    <div>
                        ...
                        <small class="text-muted">No. RM: {{ $rekamMedis->no_rm_kunjungan }}</small>
                    </div>
                </div>
            </div>
            <div class="card-body-custom">
                        ...
                        <div class="p-3 bg-light rounded-3 h-100">
                            ...
                            <table class="table table-borderless mb-0">
                                <tr>
                                    ...
                                    <td class="py-1"><strong>{{ $rekamMedis->pasien->no_rm ?? '-' }}</strong></td>
                                </tr>
                                <tr>
                                    <td class="text-muted-small py-1">Nama Pasien</td>
                                    <td class="py-1"><strong>{{ $rekamMedis->pasien->nama_lengkap ?? '-' }}</strong></td>
                                </tr>
                                <tr>
                                    <td class="text-muted-small py-1">Tanggal Lahir</td>
                                    <td class="py-1">{{ $rekamMedis->pasien->tanggal_lahir ? \Carbon\Carbon::parse($rekamMedis->pasien->tanggal_lahir)->format('d-m-Y') : '-' }}</td>
                                </tr>
                                <tr>
                                    <td class="text-muted-small py-1">Jenis Kelamin</td>
                                    <td class="py-1">{{ $rekamMedis->pasien->jenis_kelamin == 'L' ? 'Laki-laki' : 'Perempuan' }}</td>
                                </tr>
                            </table>
                        </div>
                    </div>
                        ...
                        <div class="p-3 bg-light rounded-3 h-100">
                            ...
                            <table class="table table-borderless mb-0">
                                <tr>
                                    ...
                                    <td class="py-1"><strong>{{ $rekamMedis->kunjungan->no_kvisitasi ?? '-' }}</strong></td>
                                </tr>
                                <tr>
                                    <td class="text-muted-small py-1">Tanggal Periksa</td>
                                    <td class="py-1">{{ \Carbon\Carbon::parse($rekamMedis->tanggal_periksa)->format('d-m-Y H:i') }}</td>
                                </tr>
                                <tr>
                                    <td class="text-muted-small py-1">Dokter</td>
                                    <td class="py-1">{{ $rekamMedis->dokter->nama_lengkap ?? '-' }}</td>
                                </tr>
                                <tr>
                                    <td class="text-muted-small py-1">Poli</td>
                                    <td class="py-1">{{ $rekamMedis->kunjungan->poli->nama_poli ?? '-' }}</td>
                                </tr>
                            </table>
                        </div>
                    </div>
                ...
                <form method="POST" action="{{ route('rekam_medis.update', $rekamMedis->id) }}">
                        ...
                        <div class="p-4 bg-light rounded-3">
                            <div class="row g-3">
                                <div class="col-12">
                                    ...
                                    <textarea name="anamnesis" class="form-control" rows="3" required>{{ $rekamMedis->anamnesis }}</textarea>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold">Riwayat Penyakit Dahulu</label>
                                    <input type="text" name="riwayat_penyakit_dahulu" class="form-control" value="{{ $rekamMedis->riwayat_penyakit_dahulu }}">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold">Riwayat Penyakit Keluarga</label>
                                    <input type="text" name="riwayat_penyakit_keluarga" class="form-control" value="{{ $rekamMedis->riwayat_penyakit_keluarga }}">
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label fw-semibold">Riwayat Alergi</label>
                                    <input type="text" name="riwayat_alergi" class="form-control" value="{{ $rekamMedis->riwayat_alergi }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold">Riwayat Obat Rutin</label>
                                    <input type="text" name="riwayat_obat_rutin" class="form-control" value="{{ $rekamMedis->riwayat_obat_rutin }}">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-semibold">Catatan Dokter</label>
                                    <input type="text" name="catatan_dokter" class="form-control" value="{{ $rekamMedis->catatan_dokter }}">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="p-4 bg-light rounded-3">
                            <div id="diagnosa-container">
                                @foreach($rekamMedis->diagnosas as $index => $diagnosa)
                                <div class="row g-3 mb-2 diagnosa-row">
                                    <div class="col-md-5">
                                        ...
                                        <select name="icd10_id[]" class="form-select select2">
                                            ...
                                            @foreach($icd10s as $icd10)
                                                <option value="{{ $icd10->id }}" {{ $diagnosa->icd10_id == $icd10->id ? 'selected' : '' }}>
                                                    {{ $icd10->kode }} - {{ $icd10->nama_penyakit_indonesia }}
                                                </option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-md-4">
                                        ...
                                        <select name="jenis_diagnosa[]" class="form-select">
                                            <option value="utama" {{ $diagnosa->jenis == 'utama' ? 'selected' : '' }}>Utama</option>
                                            <option value="sekunder" {{ $diagnosa->jenis == 'sekunder' ? 'selected' : '' }}>Sekunder</option>
                                            <option value="komplikasi" {{ $diagnosa->jenis == 'komplikasi' ? 'selected' : '' }}>Komplikasi</option>
                                        </select>
                                    </div>
                                    ...
                                    <input type="hidden" name="diagnosa_ids[]" value="{{ $diagnosa->id }}">
                                </div>
                                @endforeach
                            </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="p-4 bg-light rounded-3">
                            <div id="tindakan-container">
                                @foreach($rekamMedis->tindakanMedis as $index => $tindakan)
                                <div class="row g-3 mb-2 tindakan-row">
                                    <div class="col-md-4">
                                        ...
                                        <select name="tindakan_id[]" class="form-select select2">
                                            ...
                                            @foreach($tindakans as $t)
                                                <option value="{{ $t->id }}" {{ $tindakan->master_tindakan_id == $t->id ? 'selected' : '' }}>
                                    ...
                                    <div class="col-md-2">
                                        ...
                                        <input type="number" name="jumlah_tindakan[]" class="form-control" value="{{ $tindakan->jumlah }}" min="1">
                                    </div>
                                    <div class="col-md-4">
                                        <label class="form-label">Keterangan</label>
                                        <input type="text" name="keterangan_tindakan[]" class="form-control" value="{{ $tindakan->keterangan }}">
                                    </div>
                                    <div class="col-md-2 d-flex align-items-end">
                                    ...
                                    <input type="hidden" name="tindakan_ids[]" value="{{ $tindakan->id }}">
                                </div>
                                @endforeach
                            </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="p-4 bg-light rounded-3">
                            <textarea name="rencana_tindak_lanjut" class="form-control" rows="2">{{ $rekamMedis->rencana_tindak_lanjut }}</textarea>
                        </div>
                    </div>

                    <div class="d-flex justify-content-end gap-2">
                        <a href="{{ route('rekam_medis.show', $rekamMedis->id) }}" class="btn-action btn-action-secondary">
                            <i class="fas fa-times"></i> Batal
                        </a>
                        <button type="submit" class="btn-custom btn-custom-primary">
                            <i class="fas fa-save"></i> Update Rekam Medis
                        </button>
                    </div>
                </form>
            </div>
